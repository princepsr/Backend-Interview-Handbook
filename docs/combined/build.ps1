...
$volumes = @(
    ...
    @{
        Num      = 5
        VolDir   = 'Volume5_SystemDesign_LLD'
...
function Build-Volume($vol) {
    ...
    # 3b. Copy this volume's dedicated study guide and company guide (replaces global ones)
    $volStudySrc   = Join-Path $root "$($vol.VolDir)\STUDY_GUIDE.md"
    $volCompanySrc = Join-Path $root "$($vol.VolDir)\COMPANY_GUIDE.md"
    if (Test-Path $volStudySrc)   { Copy-Item $volStudySrc   (Join-Path $tmpDir 'STUDY_GUIDE.md') }
    if (Test-Path $volCompanySrc) { Copy-Item $volCompanySrc (Join-Path $tmpDir 'COMPANY_GUIDE.md') }

    # 3c. Copy shared images directory
    $imagesDir = Join-Path $root 'images'
    if (Test-Path $imagesDir) {
        $destImages = Join-Path $tmpDir 'images'
        Copy-Item -Recurse -Force $imagesDir $destImages
    }
    ...
    $lines.Add('---')
    $lines.Add('')
    $lines.Add('# Study Guides')
    $lines.Add('')
    $lines.Add('- [Study Plan](STUDY_GUIDE.md)')
    $lines.Add('- [Company Guide](COMPANY_GUIDE.md)')
    $lines.Add('')
    $lines.Add('---')
    $lines.Add('')
    $lines.Add("# $($vol.Title)")
    ...
    $tomlLines.Add('')
    # [output.pdf] disabled — headless Chrome times out on large books
    # To re-enable: uncomment the lines below and ensure Chrome is on PATH
